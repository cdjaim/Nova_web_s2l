...
document.addEventListener("click", (e) => {             //click out menu
    const rotateArrow = document.querySelectorAll(".arrow");
    const menuPrincipal = document.querySelector("#menu-list");

    if (!menuPrincipal.contains(e.target)) {
        rotateArrow.forEach((itemmenu) => {
            itemmenu.classList.remove("arrow");
